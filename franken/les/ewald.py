...
class Ewald(nn.Module):
    ...
    def forward(
        ...
        alpha: Optional[
            torch.Tensor
        ] = None,  # [n_atoms, n_q] or [n_atoms, n_q, 3, 3] or [n_atoms] or [n_atoms, 3, 3]
        ...
        for i in unique_batches.long():
            ...
            box_now = (
                cell[i] if cell is not None else None
            )  # Get the box for the i-th configuration
            ...
            if box_now is None or torch.linalg.det(box_now) < 1e-6:
                ...
                result = self.compute_potential_realspace(
                    r_raw=r_raw_now,
                    q=q_now,
                    u=u_now,
                    quad=quad_now,
                ...
                result = self.compute_potential_triclinic(
                    r_raw=r_raw_now,
                    q=q_now,
                    cell_now=box_now,
                    u=u_now,
        ...
        return (
            torch.cat(results),
            torch.cat(q_induced_results),
            torch.cat(u_induced_results),
        )

    def compute_potential_realspace(
        self,
        r_raw,
        q,
        ...
        if not self.remove_self_interaction:
            ...
            if u is not None:
                pot += (
                    (u**2).sum(dim=(0, 2))
                    / (3 * self.sigma**3.0 * self.twopi**1.5)
                    * self.norm_factor
                )
            ...
            if quad is not None:
                pot += (
                    (quad**2).sum(dim=(0, 2, 3))
                    / (10 * self.sigma**5.0 * self.twopi**1.5)
                    * self.norm_factor
                )
    ...
    def _get_epsilon_r(self, alpha, volume):
        ...
        if alpha.dim() == 2:  # isotropic alpha
            epsilon_r = alpha.sum(axis=0) / volume / epsilon_0 + 1.0
        elif alpha.dim() == 4 and alpha.shape[2:4] == (3, 3):  # anisotropic alpha
            epsilon_r = torch.einsum("iqcc->q", alpha) / 3.0 / volume / epsilon_0 + 1.0
        else:
            raise ValueError("alpha dimension error")
        return epsilon_r
    ...
    def compute_potential_triclinic(
        ...
        G = (
            2 * torch.pi * cell_inv.T
        )  # Reciprocal lattice vectors [3,3], G = 2π(M^{-1}).T

        if (
            alpha is not None
            and hasattr(self, "use_epsilon_r_scaling")
            and self.use_epsilon_r_scaling
        ):
        ...
        if self.remove_self_interaction:
            pot -= (
                torch.sum(q**2, dim=0)
                / (self.sigma * self.twopi**1.5)
                * self.norm_factor
            )
            if u is not None:
                pot -= (
                    torch.sum(u**2, dim=(0, 2))
                    / (3 * self.sigma**3.0 * self.twopi**1.5)
                    * self.norm_factor
                )
            if quad is not None:
                pot -= (
                    torch.sum(quad**2, dim=(0, 2, 3))
                    / (10 * self.sigma**5.0 * self.twopi**1.5)
                    * self.norm_factor
                )
        ...
        if compute_potential or kappa is not None:
            ...
            term_real = S_k_real.unsqueeze(0) * cos_kr.unsqueeze(
                1
            ) + S_k_imag.unsqueeze(0) * sin_kr.unsqueeze(
                1
            )  # [n, n_q, M]
            ...
            if self.remove_self_interaction:
                e_phi -= (
                    q * (2 / (self.sigma * self.twopi**1.5)) * self.norm_factor
                )  # [n, n_q]
        ...
        if compute_field or alpha is not None:
            ...
            term_imag = S_k_real.unsqueeze(0) * sin_kr.unsqueeze(
                1
            ) - S_k_imag.unsqueeze(0) * cos_kr.unsqueeze(
                1
            )  # [n, n_q, M]
            e_field = (
        ...
        output = {
            "pot": pot.sum().view(
                -1
            ),  # sum over the energy contributions from different nq channels
            "q_induced": q_induced.squeeze(dim=1) if one_dim_input else q_induced,
            "u_induced": u_induced.squeeze(dim=1) if one_dim_input else u_induced,
            "phi": e_phi,
            "field": e_field,
