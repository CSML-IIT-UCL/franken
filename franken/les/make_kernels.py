...
def make_kernels(r, sigma, norm_const, compute_u=True, compute_Q=True):
    ...
    f_uu = (
        s2[:, :, None, None] * rr - s1[:, :, None, None] * eye[None, None]
    ) * norm_const  # [n,n,3,3]
    ...
    if compute_Q:
        ...
        term_delta_r = (
            torch.einsum("ab,ijc->ijabc", eye, rhat)
        ...
        ) * norm_const  # [n,n,3,3,3]

        rrrr = torch.einsum("ija,ijb,ijc,ijd->ijabcd", rhat, rhat, rhat, rhat)
        term_delta_rr = (
            torch.einsum("ab,ijc,ijd->ijabcd", eye, rhat, rhat)
...
def bare_multipole_tensors(r):
    ...
    T4 = (105.0 * rrrr - 15.0 * delta_rr + 3.0 * delta_delta) * rinv5[
        ..., None, None, None, None
    ]  # [n,n,3,3,3,3]
