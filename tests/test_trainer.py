import json
import logging
import pathlib
from unittest.mock import DEFAULT, MagicMock, Mock, mock_open, patch

import pytest
import torch

from franken.config import GaussianRFConfig, MaceBackboneConfig
from franken.data.base import ENERGY_TARGET_KEY, FORCES_TARGET_KEY, STRESS_TARGET_KEY, Configuration, Target, TargetType
from franken.rf.model import FrankenPotential
from franken.rf.scaler import Statistics
from franken.trainers import RandomFeaturesTrainer
from franken.trainers.log_utils import DataSplit, LogCollection, LogEntry
from franken.trainers.rf_trainer import process_tgt_weights
from franken.trainers.rf_lowmem import LowMemRandomFeaturesTrainer
from tests.utils import mocked_gnn
from .conftest import DEVICES


def gen_efmap_vals(num_feat, num_configs, num_atoms, device, dtype, targets):
    out = []
    for _ in range(num_configs):
        od = {}
        if ENERGY_TARGET_KEY in targets:
            od[ENERGY_TARGET_KEY] = torch.randn(num_feat, 1, device=device, dtype=dtype)
        if FORCES_TARGET_KEY in targets:
            od[FORCES_TARGET_KEY] = torch.randn(num_feat, num_atoms * 3, device=device, dtype=dtype)
        if STRESS_TARGET_KEY in targets:
            od[STRESS_TARGET_KEY] = torch.randn(num_feat, 3 * 3, device=device, dtype=dtype)
        out.append(od)
    return out


def gen_data(num_configs, num_atoms, dtype, split="train", gen_stress=True):
    data = [
        (
            Configuration(
                torch.randn(num_atoms, 3, dtype=dtype), 
                torch.randint(1, 90, (num_atoms, )), 
                torch.tensor(num_atoms),
                edge_index=torch.randint(0, num_atoms, (num_atoms * 2, 2)),
                unit_shifts=torch.randint(0, 5, (num_atoms * 2, 3), dtype=torch.int32),
                cell=torch.randn((3, 3), dtype=dtype)
            ),
            Target(
                torch.randn(1) ** 2, 
                torch.randn(num_atoms, 3) ** 2, 
                torch.randn(3, 3) ** 2 if gen_stress else None
            )
        )
        for _ in range(num_configs)
    ]
    def iterator(*args, **kwargs):
        for x in data:
            yield x
    dl_mock = MagicMock()
    dl_mock.__len__ = MagicMock(return_value=num_configs)
    dl_mock.__iter__ = iterator
    dl_mock.dataset.__len__ = MagicMock(return_value=num_configs)
    dl_mock.dataset.split = split
    return dl_mock


def init_mock_model(efmaps, device, dtype):
    # We need the following from the potential model
    # - model.rf.total_random_features
    # - model.grad_feature_map(data)
    with mocked_gnn(device=device, dtype=dtype, feature_dim=32):
        underlying = FrankenPotential(
            gnn_config="test",  # pyright: ignore[reportArgumentType]
            rf_config=GaussianRFConfig(num_random_features=64, length_scale=1.0)
        ).to(device)
        model = Mock(wraps=underlying)
        model.rf = Mock(wraps=underlying.rf)
        if efmaps is not None:
            # will iterate through efmap_vals at every call
            model.grad_feature_map = MagicMock(side_effect=efmaps)
            model.rf.total_random_features = efmaps[0][FORCES_TARGET_KEY].shape[0]
        else:
            model.rf.total_random_features = underlying.rf.total_random_features
        return model


@pytest.mark.parametrize("device", DEVICES)
class TestLowMemTrainer:
    EF_TGT = [ENERGY_TARGET_KEY, FORCES_TARGET_KEY]
    num_configs = 3
    num_atoms = 10
    num_rf = 4

    def trainer(self, device, dtype=torch.float32, **kwargs):
        return LowMemRandomFeaturesTrainer(
            # it's okay to pass None here as long as we don't test `fit` or `set_statistics`
            train_dataloader=None, # type: ignore
            l2_penalty=kwargs.pop("l2_penalty", [1.0]),
            target_weight=kwargs.pop("target_weight", {}),
            training_targets=kwargs.pop("training_targets", [ENERGY_TARGET_KEY, FORCES_TARGET_KEY]),
            random_features_normalization=None,
            log_dir=None,
            save_every_model=False,
            device=device,
            dtype=dtype,
            **kwargs
        )

    def test_init_incorrect_tgts(self, device):
        with pytest.raises(ValueError, match="The low-memory trainer only works with 2 training targets."):
            self.trainer(device, torch.float32, training_targets=[ENERGY_TARGET_KEY])
        with pytest.raises(ValueError, match="The low-memory trainer only works with 2 training targets."):
            self.trainer(device, torch.float32, training_targets=[ENERGY_TARGET_KEY, STRESS_TARGET_KEY, FORCES_TARGET_KEY])

    def test_ok_more_data_provided(self, device):
        trainer = self.trainer(device, torch.float32)
        efmap_vals = gen_efmap_vals(self.num_rf, self.num_configs, self.num_atoms, device, trainer.buffer_dt,
                                        [ENERGY_TARGET_KEY, FORCES_TARGET_KEY, STRESS_TARGET_KEY])
        dataloader = gen_data(self.num_configs, self.num_atoms, trainer.buffer_dt)
        model = init_mock_model(efmap_vals, device, trainer.buffer_dt)
        covs, coeffs = trainer._covs_and_coeffs(model, dataloader)
        assert ENERGY_TARGET_KEY in covs
        assert FORCES_TARGET_KEY in covs
        assert ENERGY_TARGET_KEY in coeffs
        assert FORCES_TARGET_KEY in coeffs

    def test_err_not_enough_data(self, device):
        trainer = self.trainer(device, torch.float32)
        efmap_vals = gen_efmap_vals(self.num_rf, self.num_configs, self.num_atoms, device, trainer.buffer_dt,
                                        [FORCES_TARGET_KEY])
        dataloader = gen_data(self.num_configs, self.num_atoms, trainer.buffer_dt)
        model = init_mock_model(efmap_vals, device, trainer.buffer_dt)
        with pytest.raises(KeyError):
            trainer._covs_and_coeffs(model, dataloader)

    def test_covs_and_coeffs(self, device):
        trainer = self.trainer(device, torch.float32)
        efmap_vals = gen_efmap_vals(self.num_rf, self.num_configs, self.num_atoms, device, trainer.buffer_dt, self.EF_TGT)
        dataloader = gen_data(self.num_configs, self.num_atoms, trainer.buffer_dt)
        # Expected energy and forces covariance and coeffs
        exp_f_cov = sum([efmap[FORCES_TARGET_KEY] @ efmap[FORCES_TARGET_KEY].T for efmap in efmap_vals])
        exp_f_coef = sum([efmap[FORCES_TARGET_KEY] @ data[1].forces.to(device).view(-1) / self.num_atoms for efmap, data in zip(efmap_vals, dataloader)])
        exp_e_cov = sum([efmap[ENERGY_TARGET_KEY] @ efmap[ENERGY_TARGET_KEY].T for efmap in efmap_vals])
        exp_e_coef = sum([efmap[ENERGY_TARGET_KEY].view(-1) * data[1].energy.to(device) / self.num_atoms for efmap, data in zip(efmap_vals, dataloader)])

        model = init_mock_model(efmap_vals, device, trainer.buffer_dt)
        covs, coeffs = trainer._covs_and_coeffs(model, dataloader)
        cov = covs[ENERGY_TARGET_KEY][0]
        diag_energy = covs[ENERGY_TARGET_KEY][1]
        diag_forces = covs[FORCES_TARGET_KEY][1]
        # Check outputs
        torch.testing.assert_close(torch.triu(cov, 1), torch.triu(exp_e_cov, 1))
        torch.testing.assert_close(diag_energy, torch.diagonal(exp_e_cov))
        torch.testing.assert_close(torch.tril(cov, -1), torch.tril(exp_f_cov, -1))
        torch.testing.assert_close(diag_forces, torch.diagonal(exp_f_cov))
        torch.testing.assert_close(coeffs[ENERGY_TARGET_KEY], exp_e_coef)
        torch.testing.assert_close(coeffs[FORCES_TARGET_KEY], exp_f_coef)
        # Check behavior
        assert model.grad_feature_map.call_count == self.num_configs

    def test_covs_and_coeffs_stress(self, device):
        ttype = [STRESS_TARGET_KEY, FORCES_TARGET_KEY]
        trainer = self.trainer(device, torch.float32, training_targets=ttype)
        efmap_vals = gen_efmap_vals(self.num_rf, self.num_configs, self.num_atoms, device, trainer.buffer_dt, ttype)
        dataloader = gen_data(self.num_configs, self.num_atoms, trainer.buffer_dt)
        # Expected energy and forces covariance and coeffs
        exp_f_cov = sum([efmap[FORCES_TARGET_KEY] @ efmap[FORCES_TARGET_KEY].T for efmap in efmap_vals])
        exp_f_coef = sum([efmap[FORCES_TARGET_KEY] @ data[1].forces.to(device).view(-1) / self.num_atoms for efmap, data in zip(efmap_vals, dataloader)])
        exp_s_cov = sum([efmap[STRESS_TARGET_KEY] @ efmap[STRESS_TARGET_KEY].T for efmap in efmap_vals])
        exp_s_coef = sum([efmap[STRESS_TARGET_KEY] @ data[1].stress.to(device).view(-1) / self.num_atoms for efmap, data in zip(efmap_vals, dataloader)])

        model = init_mock_model(efmap_vals, device, trainer.buffer_dt)
        covs, coeffs = trainer._covs_and_coeffs(model, dataloader)
        cov = covs[STRESS_TARGET_KEY][0]
        diag_stress = covs[STRESS_TARGET_KEY][1]
        diag_forces = covs[FORCES_TARGET_KEY][1]
        # Check outputs
        torch.testing.assert_close(torch.triu(cov, 1), torch.triu(exp_s_cov, 1))
        torch.testing.assert_close(diag_stress, torch.diagonal(exp_s_cov))
        torch.testing.assert_close(torch.tril(cov, -1), torch.tril(exp_f_cov, -1))
        torch.testing.assert_close(diag_forces, torch.diagonal(exp_f_cov))
        torch.testing.assert_close(coeffs[STRESS_TARGET_KEY], exp_s_coef)
        torch.testing.assert_close(coeffs[FORCES_TARGET_KEY], exp_f_coef)
        # Check behavior
        assert model.grad_feature_map.call_count == self.num_configs

    def test_solve(self, device):
        # Start by computing covs and coeffs
        trainer = self.trainer(device, torch.float32)
        force_weight = 0.1
        l2_penalty = 1e-6
        num_rf = 32
        efmap_vals = gen_efmap_vals(num_rf, self.num_configs, self.num_atoms, device, trainer.buffer_dt, self.EF_TGT)
        dataloader = gen_data(self.num_configs, self.num_atoms, trainer.buffer_dt)
        # Expected energy and forces covariance and coeffs
        exp_f_cov = sum([efmap[FORCES_TARGET_KEY] @ efmap[FORCES_TARGET_KEY].T for efmap in efmap_vals])
        exp_f_coef = sum([efmap[FORCES_TARGET_KEY] @ data[1].forces.to(device).view(-1) / self.num_atoms for efmap, data in zip(efmap_vals, dataloader)])
        exp_e_cov = sum([efmap[ENERGY_TARGET_KEY] @ efmap[ENERGY_TARGET_KEY].T for efmap in efmap_vals])
        exp_e_coef = sum([efmap[ENERGY_TARGET_KEY].view(-1) * data[1].energy.to(device) / self.num_atoms for efmap, data in zip(efmap_vals, dataloader)])
        lerped_cov = exp_f_cov * force_weight + exp_e_cov * (1 - force_weight)
        exp_sol = torch.linalg.solve(
            lerped_cov + torch.eye(num_rf, dtype=trainer.buffer_dt, device=device) * l2_penalty,
            exp_f_coef * force_weight + exp_e_coef * (1 - force_weight)
        )
        model = init_mock_model(efmap_vals, device, trainer.buffer_dt)
        covs, coeffs = trainer._covs_and_coeffs(model, dataloader)
        solution = trainer.solve(covs, coeffs, l2_penalty=l2_penalty, 
                                 forces_weight=force_weight, energy_weight=1.0 - force_weight)
        torch.testing.assert_close(exp_sol, solution)


@pytest.mark.parametrize("device", DEVICES)
class TestTrainer:
    EF_TGT = [ENERGY_TARGET_KEY, FORCES_TARGET_KEY]
    ESF_TGT = [ENERGY_TARGET_KEY, STRESS_TARGET_KEY, FORCES_TARGET_KEY]
    num_configs = 3
    num_atoms = 10
    num_rf = 4

    def trainer(self, device, dtype=torch.float32, **kwargs):
        return RandomFeaturesTrainer(
            # it's okay to pass None here as long as we don't test `fit` or `set_statistics`
            train_dataloader=None, # type: ignore
            l2_penalty=kwargs.pop("l2_penalty", [1.0]),
            target_weight=kwargs.pop("target_weight", {}),
            training_targets=kwargs.pop("training_targets", self.EF_TGT),
            random_features_normalization=None,
            log_dir=None,
            save_every_model=False,
            device=device,
            dtype=dtype,
            **kwargs
        )
    
    def test_compute_train_preds_shortcut(self, device):
        trainer = self.trainer(device, torch.float64, save_fmaps=True, training_targets=self.ESF_TGT)
        dataloader = gen_data(self.num_configs, self.num_atoms, trainer.buffer_dt, split="train")
        model = init_mock_model(None, device, trainer.buffer_dt)
        covs, coeffs = trainer._covs_and_coeffs(model, dataloader)
        weights = trainer.solve(covs, coeffs, l2_penalty=1e-3, 
                                forces_weight=0.3, energy_weight=1.0 - 0.3, stress_weight=0.2)
        weights = weights.view(1, -1)  # fit converts to this shape

        logs = LogCollection([LogEntry("", 0, 0, 0, [], [])])
        logs_fast = trainer.evaluate(model, dataloader, logs, weights)
        model.predict.assert_not_called()
        model._predict.assert_not_called()
        model.predict_from_fmaps.assert_called()
        model.predict_from_fmaps.reset_mock()

        trainer.save_fmaps = False
        logs = LogCollection([LogEntry("", 0, 0, 0, [], [])])
        logs_slow = trainer.evaluate(model, dataloader, logs, weights)
        model.predict.assert_called()
        model.predict_from_fmaps.assert_not_called()

        metrics_fast = logs_fast[0].metrics
        metrics_slow = logs_slow[0].metrics
        for mfast, mslow in zip(metrics_fast, metrics_slow):
            assert mfast.name == mslow.name
            torch.testing.assert_close(mslow.value, mfast.value, msg=f"Slow and fast computations don't match metric {mfast.name}")

    def test_covs_and_coeffs_stress(self, device):
        trainer = self.trainer(device, torch.float32)
        efmap_vals = gen_efmap_vals(self.num_rf, self.num_configs, self.num_atoms, device, trainer.buffer_dt, self.ESF_TGT)
        dataloader = gen_data(self.num_configs, self.num_atoms, trainer.buffer_dt)
        # Expected energy and forces covariance and coeffs
        exp_covs = {
            ENERGY_TARGET_KEY: sum([efmap[ENERGY_TARGET_KEY] @ efmap[ENERGY_TARGET_KEY].T for efmap in efmap_vals]),
            FORCES_TARGET_KEY: sum([efmap[FORCES_TARGET_KEY] @ efmap[FORCES_TARGET_KEY].T for efmap in efmap_vals]),
            STRESS_TARGET_KEY: sum([efmap[STRESS_TARGET_KEY] @ efmap[STRESS_TARGET_KEY].T for efmap in efmap_vals]),
        }
        exp_coeffs = {
            STRESS_TARGET_KEY: sum([efmap[STRESS_TARGET_KEY] @ data[1].stress.to(device).view(-1) / self.num_atoms for efmap, data in zip(efmap_vals, dataloader)]),
            FORCES_TARGET_KEY: sum([efmap[FORCES_TARGET_KEY] @ data[1].forces.to(device).view(-1) / self.num_atoms for efmap, data in zip(efmap_vals, dataloader)]),
            ENERGY_TARGET_KEY: sum([efmap[ENERGY_TARGET_KEY].view(-1) * data[1].energy.to(device) / self.num_atoms for efmap, data in zip(efmap_vals, dataloader)]),
        }
        model = init_mock_model(efmap_vals, device, trainer.buffer_dt)
        covs, coeffs = trainer._covs_and_coeffs(model, dataloader)
        # Check outputs
        for tgt in trainer.training_targets:
            torch.testing.assert_close(covs[tgt], exp_covs[tgt])
            torch.testing.assert_close(coeffs[tgt], exp_coeffs[tgt])
        # Check behavior
        assert model.grad_feature_map.call_count == self.num_configs

    def test_solve(self, device):
        # Start by computing covs and coeffs
        trainer = self.trainer(device, torch.float32, training_targets=self.ESF_TGT)
        forces_weight = 0.1
        stress_weight = 0.8
        energy_weight = 0.1
        l2_penalty = 1e-6
        num_rf = 32
        efmap_vals = gen_efmap_vals(num_rf, self.num_configs, self.num_atoms, device, trainer.buffer_dt, self.ESF_TGT)
        dataloader = gen_data(self.num_configs, self.num_atoms, trainer.buffer_dt)
        # Expected energy and forces covariance and coeffs
        exp_covs = {
            ENERGY_TARGET_KEY: sum([efmap[ENERGY_TARGET_KEY] @ efmap[ENERGY_TARGET_KEY].T for efmap in efmap_vals]),
            FORCES_TARGET_KEY: sum([efmap[FORCES_TARGET_KEY] @ efmap[FORCES_TARGET_KEY].T for efmap in efmap_vals]),
            STRESS_TARGET_KEY: sum([efmap[STRESS_TARGET_KEY] @ efmap[STRESS_TARGET_KEY].T for efmap in efmap_vals]),
        }
        exp_coeffs = {
            STRESS_TARGET_KEY: sum([efmap[STRESS_TARGET_KEY] @ data[1].stress.to(device).view(-1) / self.num_atoms for efmap, data in zip(efmap_vals, dataloader)]),
            FORCES_TARGET_KEY: sum([efmap[FORCES_TARGET_KEY] @ data[1].forces.to(device).view(-1) / self.num_atoms for efmap, data in zip(efmap_vals, dataloader)]),
            ENERGY_TARGET_KEY: sum([efmap[ENERGY_TARGET_KEY].view(-1) * data[1].energy.to(device) / self.num_atoms for efmap, data in zip(efmap_vals, dataloader)]),
        }
        lerped_cov = (
            exp_covs[ENERGY_TARGET_KEY] * energy_weight + 
            exp_covs[FORCES_TARGET_KEY] * forces_weight + 
            exp_covs[STRESS_TARGET_KEY] * stress_weight
        )
        lerped_coef = (
            exp_coeffs[ENERGY_TARGET_KEY] * energy_weight + 
            exp_coeffs[FORCES_TARGET_KEY] * forces_weight + 
            exp_coeffs[STRESS_TARGET_KEY] * stress_weight
        )
        exp_sol = torch.linalg.solve(
            lerped_cov + torch.eye(num_rf, dtype=trainer.buffer_dt, device=device) * l2_penalty,
            lerped_coef
        )
        model = init_mock_model(efmap_vals, device, trainer.buffer_dt)
        covs, coeffs = trainer._covs_and_coeffs(model, dataloader)
        solution = trainer.solve(covs, coeffs, l2_penalty=l2_penalty, 
                                 forces_weight=forces_weight, energy_weight=energy_weight, 
                                 stress_weight=stress_weight)
        torch.testing.assert_close(exp_sol, solution)

    def test_missing_target_type(self, device):
        trainer = self.trainer(device, torch.float32, training_targets=self.ESF_TGT)
        efmap_vals = gen_efmap_vals(self.num_rf, self.num_configs, self.num_atoms, device, trainer.buffer_dt, self.ESF_TGT)
        dataloader = gen_data(self.num_configs, self.num_atoms, trainer.buffer_dt, gen_stress=False)
        model = init_mock_model(efmap_vals, device, trainer.buffer_dt)
        with pytest.raises(RuntimeError, match="does not contain any values for stress."):
            covs, coeffs = trainer._covs_and_coeffs(model, dataloader)
        

class TestSerializeBestModel:
    @pytest.fixture
    def log_collection(self):
        return LogCollection(logs=[LogEntry.from_dict(log) for log in [{
            "checkpoint": {"hash": "rand_uuid", "rf_weight_id": 0},
            "timings": {"cov_coeffs": 1.0, "solve": 1.0},
            "metrics": {
                "train": {"energy_MAE": 1.0, "forces_MAE": 1.0, "forces_cosim": 1.0},
                "validation": {"energy_MAE": 1.0, "forces_MAE": 1.0, "forces_cosim": 1.0},
                "test": {"energy_MAE": 1.0, "forces_MAE": 1.0, "forces_cosim": 1.0},
            },
            "hyperparameters": {
                "franken": {
                    "gnn_backbone_id": "mace_mp/small",
                    "interaction_block": 3,
                    "kernel_type": "gaussian",
                },
                "random_features": {
                    "num_random_features": 1024,
                },
                "input_scaler": {"scale_by_Z": True, "num_species": 2},
                "solver": {
                    "l2_penalty": 1e-6,
                    "forces_weight": 0.1,
                    "dtype": "torch.float64",
                },
            },
        },{
            "checkpoint": {"hash": "rand_uuid2", "rf_weight_id": 1},
            "timings": {"cov_coeffs": 1.0, "solve": 1.0},
            "metrics": {
                "train": {"energy_MAE": 1.0, "forces_MAE": 1.0, "forces_cosim": 1.0},
                "validation": {"energy_MAE": 1.0, "forces_MAE": 0.5, "forces_cosim": 1.0},
                "test": {"energy_MAE": 1.0, "forces_MAE": 1.0, "forces_cosim": 1.0},
            },
            "hyperparameters": {
                "franken": {
                    "gnn_backbone_id": "mace_mp/small",
                    "interaction_block": 3,
                    "kernel_type": "gaussian",
                },
                "random_features": {
                    "num_random_features": 1024,
                },
                "input_scaler": {"scale_by_Z": True, "num_species": 2},
                "solver": {
                    "l2_penalty": 1e-6,
                    "forces_weight": 0.1,
                    "dtype": "torch.float64",
                },
            },
        }]])

    def test_should_save(self, log_collection, caplog):
        with patch.object(pathlib.Path, 'exists') as mock_exists:
            with mocked_gnn("cpu", torch.float32):
                with patch.multiple("franken.trainers.base", LogCollection=DEFAULT) as p2:
                    with patch.multiple("torch", load=DEFAULT, save=DEFAULT) as p3:
                        with patch("builtins.open", mock_open()) as file_open:
                            trainer = RandomFeaturesTrainer(
                                train_dataloader=None, # type: ignore
                                training_targets=[ENERGY_TARGET_KEY, FORCES_TARGET_KEY],
                                l2_penalty=[1e-6],
                                target_weight={},
                                random_features_normalization=None,
                                log_dir=pathlib.Path("test_dir"),
                                save_every_model=False,
                                device="cpu",
                                dtype=torch.float32
                            )
                            model = FrankenPotential(
                                gnn_config=MaceBackboneConfig("test"),
                                rf_config=GaussianRFConfig(num_random_features=1024, length_scale=1.0)
                            ).to("cpu")
                            all_weights = torch.randn(2, 1024, dtype=trainer.buffer_dt)
                            # 1. best_model_file exists but contains same best model as log_collection
                            mock_exists.return_value = True
                            p2["LogCollection"].from_json.return_value = log_collection
                            # log_collection[1] is best model (read will be used to open best_model_file)
                            file_open.return_value.read.return_value = json.dumps(log_collection[1].to_dict())
                            with caplog.at_level(logging.DEBUG):
                                trainer.serialize_best_model(
                                    model=model,
                                    all_weights=all_weights,
                                    split=DataSplit.VALIDATION
                                )
                            p3["save"].assert_not_called()
                            assert caplog.text == ""
                            # 2. best_model_file exists and does not contain same best model as log_collection
                            mock_exists.return_value = True
                            p2["LogCollection"].from_json.return_value = log_collection
                            # log_collection[0] is not best model (read will be used to open best_model_file)
                            file_open.return_value.read.return_value = json.dumps(log_collection[0].to_dict())
                            with caplog.at_level(logging.DEBUG):
                                trainer.serialize_best_model(
                                    model=model,
                                    all_weights=all_weights,
                                    split=DataSplit.VALIDATION
                                )
                            p3["save"].assert_called_once()
                            assert "Identified new best model" in caplog.text
                            assert "Saved best model" in caplog.text
                            assert p3["save"].call_args.args[1] == pathlib.Path("test_dir/best_ckpt.pt")
                            p3["save"].reset_mock()
                            # 3. best_model_file does not exist
                            mock_exists.return_value = False
                            p2["LogCollection"].from_json.return_value = log_collection
                            with caplog.at_level(logging.DEBUG):
                                trainer.serialize_best_model(
                                    model=model,
                                    all_weights=all_weights,
                                    split=DataSplit.VALIDATION
                                )
                            p3["save"].assert_called_once()
                            assert "Identified new best model" in caplog.text
                            assert "Saved best model" in caplog.text
                            assert p3["save"].call_args.args[1] == pathlib.Path("test_dir/best_ckpt.pt")

    @pytest.mark.parametrize("dtype", [torch.float32, torch.float64])
    def test_dtype_correct(self, dtype, log_collection):
        with patch.object(pathlib.Path, 'exists') as mock_exists:
            mock_exists.return_value = False
            trainer = RandomFeaturesTrainer(
                train_dataloader=None, # type: ignore
                training_targets=[ENERGY_TARGET_KEY, FORCES_TARGET_KEY],
                l2_penalty=[1e-6],
                target_weight={},
                random_features_normalization=None,
                log_dir=pathlib.Path("test_dir"),
                save_every_model=False,
                device="cpu",
                dtype=dtype
            )
            all_weights = torch.randn(2, 1024, dtype=trainer.buffer_dt)
            with mocked_gnn("cpu", torch.float32):
                with patch.multiple("franken.trainers.base", LogCollection=DEFAULT) as p2:
                    with patch.multiple("torch", load=DEFAULT, save=DEFAULT) as p3:
                        with patch("builtins.open", mock_open()):
                            p2["LogCollection"].from_json.return_value = log_collection
                            model = FrankenPotential(
                                gnn_config=MaceBackboneConfig("test"),
                                rf_config=GaussianRFConfig(1024, 1.0)
                            ).to("cpu")
                            trainer.serialize_best_model(
                                model=model,
                                all_weights=all_weights,
                                split=DataSplit.VALIDATION
                            )
                            # check model saved with correct parameters.
                            assert p3["save"].call_args.args[1] == pathlib.Path("test_dir/best_ckpt.pt")
                            save_ckpt = p3["save"].call_args.args[0]
                            assert save_ckpt["multi_weights"] is None
                            # check model weights. This checks dtype!
                            torch.testing.assert_close(model.rf.weights, all_weights[1].view(1, -1))


def test_process_tgt_weights():
    w: dict[TargetType, float | list[float]] = {ENERGY_TARGET_KEY: 1.0}
    t: list[TargetType] = [ENERGY_TARGET_KEY]
    out = process_tgt_weights(w, t)
    assert out == {ENERGY_TARGET_KEY: [1.0]}, "Doesn't create lists correctly"

    w: dict[TargetType, float | list[float]] = {ENERGY_TARGET_KEY: [2.0]}
    t: list[TargetType] = [ENERGY_TARGET_KEY, FORCES_TARGET_KEY]
    out = process_tgt_weights(w, t)
    assert out == {ENERGY_TARGET_KEY: [2.0], FORCES_TARGET_KEY: [1.0]}, "Doesn't handle missing weights"

    w: dict[TargetType, float | list[float]] = {ENERGY_TARGET_KEY: [3.0], STRESS_TARGET_KEY: [2.0]}
    t: list[TargetType] = [ENERGY_TARGET_KEY, FORCES_TARGET_KEY]
    out = process_tgt_weights(w, t)
    assert out == {ENERGY_TARGET_KEY: [3.0], FORCES_TARGET_KEY: [1.0]}, "Doesn't handle missing weights"


def test_get_statistics():
    dloader = Mock()
    dloader.dataset = Mock()
    with patch.multiple("franken.trainers.base", compute_dataset_statistics=DEFAULT) as p2:
        p2["compute_dataset_statistics"].return_value = Statistics()
        trainer = RandomFeaturesTrainer(
            train_dataloader=dloader, # type: ignore
            training_targets=[ENERGY_TARGET_KEY, FORCES_TARGET_KEY],
            l2_penalty=[1e-6],
            target_weight={},
            random_features_normalization=None,
            log_dir=pathlib.Path("test_dir"),
            save_every_model=False,
            device="cpu",
            dtype=torch.float32
        )
        with mocked_gnn("cpu", torch.float32):
            model = FrankenPotential(
                gnn_config=MaceBackboneConfig("test"),
                rf_config=GaussianRFConfig(num_random_features=64, length_scale=1.0)
            ).to("cpu")
            trainer.get_statistics(model)
            p2["compute_dataset_statistics"].assert_called_once()
            p2["compute_dataset_statistics"].reset_mock()
            trainer.get_statistics(model)
            p2["compute_dataset_statistics"].assert_not_called()
            # Initialize a new GNN (different parameters)
            model = FrankenPotential(
                gnn_config=MaceBackboneConfig("test2"),
                rf_config=GaussianRFConfig(num_random_features=64, length_scale=1.0)
            ).to("cpu")
            trainer.get_statistics(model)
            p2["compute_dataset_statistics"].assert_called_once()
